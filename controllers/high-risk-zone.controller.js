        ...
        await dbConnection.query(sql2, [pictureInserts], (err, results, fields) => {
            if (err) {
                ...
                user_response2 = {
                    error: false,
                    data: results,
                    message: "New picture has been inserted",
                    message2: user_response
                }
            }
        })
        ...
        await dbConnection.query(sql3, [h_zone_images], (err, results, fields) => {
            if (err) {
                ...
                return res.status(201).send({
                    error: false,
                    data: results,
                    message: "New record has been inserted",
                    message2: user_response,
                    message3: user_response2
                });
            }
        })
    } catch (e) {
                    ...
                    await dbConnection.query(sql, [inserts], (err, results, fields) => {
                        ...
                        if (err) {
                            ...
                            return res.status(201).send({
                                error: false,
                                data: inserts,
                                message: 'New h_zone has been created successfully.'
                            });
                    ...
                    await dbConnection.query("UPDATE dms_high_risk_zones SET ?  WHERE h_zone_id = ?", [inserts, h_zone_id], function (error, results, fields) {
                        if (error) throw error;
                            ...
                            return res.send({
                                error: false,
                                data: results,
                                message: 'h_zone has been updated successfully.'
                            });
                        }
                    })
                } else {
