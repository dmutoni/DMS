...
module.exports.getTotalUsersByDistrictID = asyncHandler(async (req, res) => {
    ...
    district_id.trim();
    dbConnection.query("SELECT COUNT(*) AS totalUsersInDistrict FROM dms_users JOIN dms_sectors ON (dms_sectors.sector_id = dms_users.sector_id) JOIN dms_districts ON (dms_districts.district_id = dms_sectors.district_id) WHERE dms_districts.district_id = ?",
...
module.exports.getTotalUsersBySectorID = asyncHandler(async (req, res) => {
    let district_id = req.params['sector_id'];
    district_id.trim();
    dbConnection.query("SELECT COUNT(*) AS totalUsersInSector FROM dms_users JOIN dms_sectors ON (dms_sectors.sector_id = dms_users.sector_id) WHERE dms_sectors.sector_id = ?",
        [district_id], function (err, rowsFound, fields) {
            if (!err) {
                res.send({status: true, data: rowsFound});
            } else {
